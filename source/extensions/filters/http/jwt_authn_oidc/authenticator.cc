/**
 * Edited jwt_authn filter for initial jwt authentication as part of OIDC.
 */


#include "extensions/filters/http/jwt_authn_oidc/authenticator.h"
...
private:
  ...
  void doneWithStatus(const Status& status, bool jwt_good);
...
void AuthenticatorImpl::verify(Http::HeaderMap& headers, Authenticator::Callbacks* callback) {
  ...
  if (tokens.empty()) {
    if (okToBypass()) {
      doneWithStatus(Status::Ok, true);
    } else {
      doneWithStatus(Status::JwtMissed, false);
    }
    return;
  }
  ...
  if (!token_->isIssuerSpecified(jwt_.iss_)) {
    ...
    doneWithStatus(Status::JwtUnknownIssuer, false);
    return;
  }
  ...
  if (jwt_.exp_ > 0 && jwt_.exp_ < unix_timestamp) {
    doneWithStatus(Status::JwtExpired, false);
    return;
  }
  ...
  if (!jwks_data_->areAudiencesAllowed(jwt_.audiences_)) {
    doneWithStatus(Status::JwtAudienceNotAllowed, false);
    return;
  }
...
void AuthenticatorImpl::onSuccess(Http::MessagePtr&& response) {
  ...
  doneWithStatus(Status::JwksFetchFail, false);
}
...
void AuthenticatorImpl::doneWithStatus(const Status& status, bool jwt_good) {
  if (jwt_good) {
    ...
    ENVOY_LOG(debug, "Initial jwt authentication failed, completed with: {}",
              ::google::jwt_verify::getStatusString(status));
    // set flag or something to go to ext_authz
    callback_->onComplete(status);
    callback_ = nullptr;
  }
}
